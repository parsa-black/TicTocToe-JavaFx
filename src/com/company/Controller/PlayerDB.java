...
public class PlayerDB {
    ...
    public static int createUser(Player player) throws SQLException {
        ...
        makeConnection();
        statement.execute(String.format("INSERT INTO players (username , score ) value ( '%s' , %f )",
    ...
    public static void editUser(Player player) throws SQLException { //Update in sql
        ...
        statement.execute(String.format(" UPDATE players SET username = '%s' , score = %f WHERE id = %d ", player.getUsername(), player.getScore(), player.getId()));
